...
//            tv_tab_title.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            LinearLayout ll_tap = (LinearLayout) tabView.findViewById(R.id.ll_tap);
            if (isSelect)
                ll_tap.setBackgroundResource(R.drawable.round_bg_ui_pink_border_5dp);
            else
                ll_tap.setBackgroundColor(Color.parseColor("#FFFFFF"));

            TextView tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
            tab_title.setTextColor(isSelect ? mTextSelectColor : mTextUnselectColor);
            tab_title.setVisibility(isSelect ? VISIBLE : GONE);
            ImageView iv_tab_icon = (ImageView) tabView.findViewById(R.id.iv_tab_icon);
            CustomTabEntity tabEntity = mTabEntitys.get(i);
